 ...
 */
function verifyScriptExists(scriptPath: string): Result<string> {
  if (!fs.existsSync(scriptPath)) {
    return createFailure(createError(
      CrystalErrorCode.SCRIPT_NOT_FOUND,
  ...
  if (!stats.isFile()) {
    return createFailure(createError(
      CrystalErrorCode.SCRIPT_NOT_FOUND,
      `Script path is not a file: ${scriptPath}`,
      { scriptPath },
 ...
 */
function parseJSONOutput<T>(output: string): Result<T> {
  if (!isValidJSON(output)) {
    return createFailure(createError(
      CrystalErrorCode.INVALID_JSON_RESPONSE,
      "Python script did not return valid JSON",
 ...
 */
export async function executePython<T = unknown>(
    ...
    shell.end((err, code) => {
      ...
      if (timedOut) {
        resolve(createFailure(createError(
          CrystalErrorCode.TIMEOUT_ERROR,
          `Python script execution timed out after ${options.timeout}ms`,
          { scriptName: options.scriptName, timeout: options.timeout },
          [
 ...
 */
export async function executePythonWithJSON<TInput, TOutput>(
  scriptName: string,
  input: TInput,
  options?: Partial<PythonExecutionOptions>
  ...
  const writeResult = { success: true as const };
  ...
  if (execResult.data.data === undefined) {
    return createFailure(createError(
      CrystalErrorCode.INVALID_JSON_RESPONSE,
      "Python script did not return any data",
      { scriptName },
...
export function checkPythonAvailable(pythonPath: string = "python3"): Result<string> {
  ...
  if (!checkResult.success) {
    return createFailure(createError(
      ...
      { pythonPath, error: checkResult.error },
      [
