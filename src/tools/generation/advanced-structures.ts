...
import {
    Generate2DMaterialSchema,
    GenerateCageSchema,
    GenerateHighEntropyAlloySchema,
    GenerateMOFSchema,
    GeneratePrototypeSchema,
    GenerateTwistedBilayerSchema
} from "../../types/tools.js";
...
export async function generatePrototype(input: unknown): Promise<Result<any>> {
    const parsed = GeneratePrototypeSchema.safeParse(input);
    if (!parsed.success) {
        return createFailure(createError(
            CrystalErrorCode.INVALID_INPUT,
            "Invalid prototype parameters",
    ...
    if (!pythonResult.success) {
        return createFailure(createError(
            ...
            pythonResult.error?.message || "Prototype generation failed"
        ));
    }
...
export async function handleGeneratePrototype(args: unknown): Promise<any> {
...
export async function generateTwistedBilayer(input: unknown): Promise<Result<any>> {
    const parsed = GenerateTwistedBilayerSchema.safeParse(input);
    if (!parsed.success) {
        return createFailure(createError(
            CrystalErrorCode.INVALID_INPUT,
            "Invalid twisted bilayer parameters",
            { zodErrors: parsed.error.errors }
        ));
    }
    ...
    if (!pythonResult.success) {
        return createFailure(createError(
            CrystalErrorCode.GENERATION_FAILED,
            pythonResult.error?.message || "Twisted bilayer generation failed"
        ));
    }
...
export async function handleGenerateTwistedBilayer(args: unknown): Promise<any> {
...
export async function generateHighEntropyAlloy(input: unknown): Promise<Result<any>> {
    const parsed = GenerateHighEntropyAlloySchema.safeParse(input);
    if (!parsed.success) {
        return createFailure(createError(
            CrystalErrorCode.INVALID_INPUT,
            "Invalid HEA parameters",
            { zodErrors: parsed.error.errors }
        ));
    }
    ...
    if (!pythonResult.success) {
        return createFailure(createError(
            CrystalErrorCode.GENERATION_FAILED,
            pythonResult.error?.message || "HEA generation failed"
        ));
    }
...
export async function handleGenerateHighEntropyAlloy(args: unknown): Promise<any> {
...
export async function generate2DMaterial(input: unknown): Promise<Result<any>> {
    const parsed = Generate2DMaterialSchema.safeParse(input);
    if (!parsed.success) {
        return createFailure(createError(
            CrystalErrorCode.INVALID_INPUT,
            "Invalid 2D material parameters",
            { zodErrors: parsed.error.errors }
        ));
    }
    ...
    if (!pythonResult.success) {
        return createFailure(createError(
            CrystalErrorCode.GENERATION_FAILED,
            pythonResult.error?.message || "2D material generation failed"
        ));
    }
...
export async function handleGenerate2DMaterial(args: unknown): Promise<any> {
...
export async function generateMOF(input: unknown): Promise<Result<any>> {
    const parsed = GenerateMOFSchema.safeParse(input);
    if (!parsed.success) {
        return createFailure(createError(
            CrystalErrorCode.INVALID_INPUT,
            "Invalid MOF parameters",
            { zodErrors: parsed.error.errors }
        ));
    }
    ...
    if (!pythonResult.success) {
        return createFailure(createError(
            CrystalErrorCode.GENERATION_FAILED,
            pythonResult.error?.message || "MOF generation failed"
        ));
    }
...
export async function handleGenerateMOF(args: unknown): Promise<any> {
...
export async function generateCage(input: unknown): Promise<Result<any>> {
    const parsed = GenerateCageSchema.safeParse(input);
    if (!parsed.success) {
        return createFailure(createError(
            CrystalErrorCode.INVALID_INPUT,
            "Invalid cage parameters",
            { zodErrors: parsed.error.errors }
        ));
    }
    ...
    if (!pythonResult.success) {
        return createFailure(createError(
            CrystalErrorCode.GENERATION_FAILED,
            pythonResult.error?.message || "Cage generation failed"
        ));
    }
...
export async function handleGenerateCage(args: unknown): Promise<any> {
