...
export async function generateNanostructure(input: unknown): Promise<Result<any>> {
    const parsed = GenerateNanostructureSchema.safeParse(input);
    if (!parsed.success) {
        return createFailure(createError(
            CrystalErrorCode.INVALID_INPUT,
            "Invalid nanostructure parameters",
    ...
    if (parsed.data.type === "nanoribbon" && parsed.data.params.ribbon_type) {
        (pythonParams as any).type = parsed.data.params.ribbon_type;
    }
...
export async function handleGenerateNanostructure(args: unknown): Promise<any> {
