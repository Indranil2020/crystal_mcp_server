...
function formatSymmetryRelationsOutput(result) {
    ...
    if (result.path) {
        ...
        const pathStr = result.path.map((p) => `**${p.number}** (${p.symbol})`).join(" → ");
        output += `${pathStr}\n\n`;
        output += `**Path Length:** ${result.length} steps\n`;
    }
    return output;
}
export async function exploreSymmetryRelations(input) {
    const parsed = ExploreSymmetryRelationsSchema.safeParse(input);
    if (!parsed.success) {
        return createFailure(createError(CrystalErrorCode.INVALID_INPUT, "Invalid input parameters", { zodErrors: parsed.error.errors }, ["Check space group numbers (1-230)"], true));
    }
    const result = await executePythonWithJSON("symmetry_transformations.py", parsed.data, { timeout: 30000 });
    if (!result.success) {
...
export async function handleExploreSymmetryRelations(args) {
